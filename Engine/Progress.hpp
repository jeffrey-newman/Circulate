...
//
// EDITED BY JEFF NEWMAN FEB 2012
//
...
namespace SewerNet
   {
      ...
      class progress_display : private boost::noncopyable
         ...
         public:
            explicit
            progress_display(unsigned long expected_count, std::ostream & os =
            ...
            unsigned long
            operator+=(unsigned long increment)
            ...
            display_tic()
               {
                  ...
                  // The line below has problems with Eclipses debugger!!!! problems with the std::flush and eclipse output redirection to its own console.
                  do
                  {
                     m_os << '*' << std::flush;
                  }
                  while (++_tic < tics_needed);
//            do { m_os << '*'; } while ( ++_tic < tics_needed );
